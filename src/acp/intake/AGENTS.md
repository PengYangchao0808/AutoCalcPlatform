...
## OVERVIEW
Parses molecular input files (XYZ/SDF/MOL/GJF/ORCA INP/Gaussian/ORCA LOG/OUT/SMILES) into typed data models. 4 files, ~610 lines.
...
├── parsers.py    # detect_format() + format parsers + bounded SMILES fallback (823 lines)
└── storage.py    # UploadStorage — file I/O and failed-upload cleanup (64 lines)
```
...
| Format detection | `parsers.py:detect_format` | Extension-based + content heuristics; returns "xyz"|"sdf"|"mol"|"gjf"|"inp"|"log"|"smiles" |
...
| QC output parsing | `parsers.py:parse_qc_output_text` | Gaussian `Standard orientation` and ORCA Cartesian sections; uses the last geometry |
| SMILES parsing | `parsers.py:parse_smiles_list` | RDKit ETKDG embedding; lines starting with `#` are comments; 100 KiB/500-line fallback bounds and per-row `RuntimeError` isolation |
| Upload flow | `storage.py:UploadStorage` | `save_upload()` (bytes→disk, 50MB cap) + `save_normalized()` + `remove_upload()` |
...
- **No TTL eviction** — failed upload cleanup is explicit via `UploadStorage.remove_upload()`; completed-upload retention remains a separate policy
