"""Cross-state atom mapping for mechanism studies.

All atom indices in this module are 0-based.
"""
...
from .models import AtomIdentityMap
...
def _require_rdkit() -> tuple[Any, Any, Any, Any, Any]:
    try:
        ...
        raise ImportError(
            "RDKit is required for mechanism atom mapping. Install with: pip install rdkit"
        ) from exc
...
def _xyz_block(symbols: list[str], coordinates: NDArray[np.float64]) -> str:
    lines = [str(len(symbols)), "generated_by=acp.mechanism.atom_mapping"]
