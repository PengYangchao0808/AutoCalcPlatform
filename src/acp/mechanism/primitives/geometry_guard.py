"""Topology and close-contact guards for S2 scan geometries."""
...
def _build_bond_graph(
    ...
    *,
    ...
    n_atoms = len(coords)
    graph = {index: [] for index in range(n_atoms)}
...
def compare_graph_topology(
    ...
    """Compare product and candidate bond graphs."""
    ...
    for atom_i in range(n_atoms):
        for atom_j in range(atom_i + 1, n_atoms):
            ...
            if pair in product_edges:
                continue
            product_distance = GeometryUtils.calculate_distance(product_coords, atom_i, atom_j)
            candidate_distance = GeometryUtils.calculate_distance(candidate_coords, atom_i, atom_j)
