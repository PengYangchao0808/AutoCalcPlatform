"""CENSO backend wrapper — subprocess-based conformer search and refinement."""
...
class CensoBackend(QCBackend, ConformerSearcher):
    """Backend wrapping the CENSO CLI for conformer ensemble generation and refinement.
    ...
    def _resolve_preset(self, preset: str | None) -> dict[str, Any]:
        ...
        raise ValueError(
            f"Unknown CENSO preset '{preset}'. Allowed: {allowed}"
        )
    ...
    def _resolve_final_part(self, output_dir: Path) -> str | None:
    ...
    def _parse_censo_json(
        ...
        for conf_id, entry in records_data.items():
            ...
            if abs(computed_gtot - gtot) > _GTOT_TOLERANCE:
                logger.warning(
                    "gtot mismatch for %s: parsed=%.10f computed=%.10f "
                    "(diff=%.2e)",
    ...
    def _validate_xyz_json_consistency(
        ...
        if not json_keys.issubset(xyz_keys):
            ...
            raise CensoParseError(
                f"JSON keys missing in XYZ: {missing}"
            )
        ...
        *,
        ...
        if not self.is_available():
            raise CensoNotAvailableError(
                f"CENSO binary '{self._censo_path}' not found on PATH"
            )
        ...
        try:
            ...
            raise CensoExecutionError(
                f"CENSO binary not found: {self._censo_path}"
            ) from exc
        ...
        if final_part is None:
            raise CensoParseError(
                f"No CENSO output JSON found in {output_dir}. "
                f"Checked: {_PARSE_PRIORITY}"
            )
        ...
        for rec in records:
            if rec.grrho == 0.0:
                logger.debug(
                    "grrho=0 for %s — evaluate_rrho may not be active", rec.conf_id,
                )
