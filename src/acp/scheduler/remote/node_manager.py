...
_DOCTOR_SOFTWARE_SCRIPT = (
    ...
    "try:\n"
    "    from cccp.software import resolve_mpirun\n"
    "except ImportError:\n"
    "    resolve_mpirun = None\n"
    ...
    "    }\n"
    "orca_cfg = exes.get('orca') or {}\n"
    "mpi_configured = orca_cfg.get('mpi_path') or 'mpirun'\n"
    "orca_resolved = report['orca']['resolved']\n"
    "orca_dir = os.path.dirname(orca_resolved) if orca_resolved else None\n"
    "if resolve_mpirun is not None:\n"
    "    mpi_resolved = resolve_mpirun(mpi_configured, orca_dir=orca_dir)\n"
    "else:\n"
    "    mpi_candidate = os.path.expanduser(str(mpi_configured))\n"
    "    mpi_resolved = (\n"
    "        os.path.realpath(mpi_candidate)\n"
    "        if os.path.isfile(mpi_candidate) and os.access(mpi_candidate, os.X_OK)\n"
    "        else shutil.which('mpirun')\n"
    "    )\n"
    "report['mpi'] = {\n"
    "    'configured': mpi_configured,\n"
    "    'resolved': str(mpi_resolved) if mpi_resolved else None,\n"
    "    'version': detect_version('mpi', mpi_resolved) if mpi_resolved else None,\n"
    "}\n"
