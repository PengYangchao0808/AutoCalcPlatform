...
| `stage_tasks.py` | StageTask/StagePlan dataclasses, StagePlanProvider protocol (per workflow: mechanism/ensemble/energy/xtbmd_censo_energy/simple; mechanism plan lists study phases S0/S1/S2/S3/SR/S4), StageTaskStore (SQLite CRUD), StageTaskObserver (polls work dir for `.stage_*` files, mirrors to DB). |
