...
## OVERVIEW
The unified `acp` CLI, stage-based workflow pipeline, capability-driven QC backends, and generic core models. 199 files, ~74.5k lines (incl. API + scheduler + nmr). Coexists with the underlying `cccp` package (Computational Chemistry Connection Package — the QC interface library).

**Active workflows (11, registry-driven):** Confsearch, PESsearch, BatchOptimize, irc, scan, nmr (DP4/DP5), and simple (singlepoint/optimize/frequency/casscf/xtb-optimize). Retired (catalog `status:"retired"`, read-only for historical-job display): ensemble, energy, xtbmd_censo_energy, mechanism, conformer, benchmark, mech-conf, mech-step, mech-confirm, mech-chain, optfreq, optfreqsp, Lowconfirm, Highconfirm.

## STRUCTURE
```
acp/
├── cli.py              # `acp run {Confsearch|PESsearch|BatchOptimize|irc|scan|nmr|serve|simple workflows}` (≈2.9k lines)
├── __init__.py          # Package docstring only
├── __main__.py          # `python -m acp` works
├── catalog.py           # WORKFLOW_CATALOG + METHOD_META + METHOD_SCHEMAS (≈4.3k lines — retired entries kept as status:"retired"; electronic-state/CASSCF route blocks)
...
├── calculations/        # Calculation-plan primitives and engines: contracts, checkpoint, executor, plans, primitives/ (sp/opt/freq/scan/irc/casscf/thermochemistry — electronic-state gates in _common.py), pes/, batch/, irc/
...
├── api/                 # FastAPI — server, routes, v1_routes, v2_routes, schemas; mechanism_readonly (historical read-only)
└── scheduler/           # Task scheduler — jobs, manager, runner, store, provenance, stage_tasks, tasks, ... + remote/ (see scheduler/AGENTS.md)
```

## WHERE TO LOOK
| Task | Location | Notes |
|------|----------|-------|
| ACP CLI entry | `cli.py` | `acp run {Confsearch\|PESsearch\|BatchOptimize\|irc\|scan\|nmr\|serve\|casscf\|simple workflows}` dispatch (≈2.9k lines) |
...
| CASSCF capability | `backends/base.py` + `backends/orca.py` | `CASSCFCalculator` Protocol (2026-09): CASSCF/NEVPT2 SP; ORCA forwarding |
...
| Calculation primitives | `calculations/primitives/` | `run_singlepoint`, `run_optimize`, `run_frequency`, `run_scan`, `run_irc`, `run_casscf`, `ThermochemistryCalculator`; electronic-state resolution + quality gates in `_common.py` |
...
| BatchOptimize engine | `calculations/batch/engine.py` | `BatchOptimizeEngine` — per-item Opt/TS + frequency + SP + thermochemistry; item×state expansion, cache-key separation, gbw inheritance (2026-09) |
...
| Result manifest (write) | `storage/manifest.py` | Unified v2 `result_manifest.json` writer (design doc §8; electronic-state product kinds) |
...
| Simple workflows | `workflows/simple.py` | singlepoint/optimize/frequency/casscf/xtb-opt (+ scan/irc registry entries); electronic-state via --charge/--multiplicity/--spin-preset/--spin-config |
...
| API v1 routes | `api/v1_routes.py` | Job submission, molecule upload, task mgmt, stage workflows; electronic-state normalization on create_job |
