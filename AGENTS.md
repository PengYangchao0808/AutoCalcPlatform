...
| Structure-source picker frontend | `frontend/js/structure_source_picker.js` + `css/structure_source_picker.css` | ACPSourcePicker: server-side search/filter/group/sort/cursor pagination, cross-page stable multi-select (source_uid, cap 200, hidden-selection count), per-item rename/tag dialogs, batch tags with per-item results, legacy v1 fallback (404/405 → recent list), localStorage `acp.picker.view.*`; mounted at 3 entries (wizard results tab / stageBatchFromResults / s2scanLoadAssets); task rename UI = #task-rename-modal + `displayTaskName()` display layer (canonicalTaskName kept for path uses) |
