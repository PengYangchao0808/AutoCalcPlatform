...
2. 在 3D 分子预览中选择两个原子，形成待扫描的距离坐标（不要求两原子当前是化学键）；
...
距离扫描的原子选择不要求当前存在感知键：任意两个不同原子都可以作为 distance 坐标。成键/断键扫描（例如 TS 附近尚未形成或正在断裂的键）不得因“两原子不在感知键图中”被拒绝；此时 `adjacency` 元数据为空，后端仅以 INFO 日志记录该原子对及实测距离（Å）用于追溯。角度、二面角与双键双坐标扫描仍然要求所选原子链/原子对映射到感知键图，不受此放宽影响。

方向语义：`start > end`（远端 → 近端，下降）与 `start < end`（近端 → 远端，上升）均为受支持输入；校验不假定升序，步长统一取 `|end - start| / (n_points - 1)`。扫描帧严格按文件顺序编号：frame 0 始终是参考/反应物端（`endpoint_direction="start"`），与数值方向无关——下降扫描时 frame 0 对应远端长距离，反应进度仍沿帧序号单调增加。
...
B 行按用户给定的 start/end 原样写入（ORCA 接受任意起止顺序）；接口不得对 `start < end` 或扫描方向做额外假设，也不得自动交换起止值。
