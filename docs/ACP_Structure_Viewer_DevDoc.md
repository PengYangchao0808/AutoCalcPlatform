...
- **高度契约**：`.sv-canvas-col` 必须 `flex: 1 1 auto` 撑满 `#sv-layout` 剩余高度（viewer 内容全部绝对定位，不贡献内容高度，缺 flex-grow 会导致画布 0px 高——2026-09-11 回归）；`>=1920px` 媒体块必须显式 `width: 100%` 配合 `max-width: 1600px; margin: 0 auto`（auto margin 关闭 flex 交叉轴拉伸，缺 width 会收缩为 fit-content）
...
| `frontend/css/structure_viewer.css` | 0.8.1 | 单列 flex 工作区（`.sv-layout { display:flex; flex-direction:column }`；`.sv-canvas-col { flex:1 1 auto }` 高度契约；≥1920px `width:100% + max-width:1600px` 居中）；summary bar / bottom strip / overlay drawers / switcher dropdown / 输入结果切换 / 播放条 / 降级通知 / 焦点可见样式；1100px 断点抽屉响应式 |
