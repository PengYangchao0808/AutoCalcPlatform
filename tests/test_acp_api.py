...
def test_status_returns_full_schema(client: TestClient) -> None:
    ...
    body = r.json()
    for field in ("service", "version", "status", "host", "port", "python",
                  "run_root", "uptime_seconds", "queue", "wsl"):
...
def test_backends_use_real_capability_matrix(client: TestClient) -> None:
    ...
    assert crest_caps.get("conformer_search") is True, "crest should expose conformer_search"


def test_workflows_and_protocols(client: TestClient) -> None:
    wf = client.get("/api/workflows").json()
    assert {"fake", "conformer", "nmr", "benchmark"} <= {w["name"] for w in wf["workflows"]}
...
def test_cancel_queued_job(client: TestClient) -> None:
    ...
    with TestClient(app) as c:
        ...
        second_id = c.post(
            "/api/jobs", json={"workflow": "fake", "input": {"source": "Y"}}
        ).json()["job_id"]
